...
function createSubCollection(Child, parent) {
  // Saving a sub collection should call the save on the parent.
  Child.prototype.save = Child.prototype.save.bind(parent);

  // Resolve push to add so that the sub collection array will be updated, in
  // stead of calling the push defined by Fossa.
  Child.prototype.push = Child.prototype.add;
...
module.exports = function collection(options) {
  ...
  , set: function set(query, docs, options, fn) {
    ...
    /**
     * Add models to a sub collection / or data to an array.
     *
     * @param {Object} query
     * @param {String} key to update
     * @param {Mixed} data array or data single object
     * @param {Object} options
     * @param {Function} fn callback
     * @api public
     */
  , push: function push(query, key, data, options, fn) {
      options = (fn = fn || options) && typeof options !== 'object' ? {} : options;

      // Get the model from the collection or database.
      this.findOne(query, function found(model) {
        model.attributes[key].push(data);
        ...
        this.getClient(function returnClient(err, client) {
          ...
          client.update(query, { $push: update }, options, fn);
        });
      });
    }
