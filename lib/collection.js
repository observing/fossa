...
module.exports = function collection(options) {
  ...
  , initialize: function initialize() {
      if (!this.name) return this.fossa.emit('error', new Error(
        'Please specify a collection name by using the `name` key'
      ));

      // Store reference to fossa.
      this.fossa = options.fossa;
    }
  ...
  , getClient: function getClient(fn) {
      this.fossa.connect(this.database, this.name, function connect(err, client) {
