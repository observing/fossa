...
module.exports = function model(fossa) {
  var Model = backbone.Model.extend({
    ...
    constructor: function constructor(attributes, options) {
      ...
      //
      // If the provided `_id` is a close resemblence of an ObjectID assume
      // the model is stored already. Passing an ObjectID otherwise is useless,
      // as both Fossa and MongoDB will provide an ObjectID by default.
      //
      if ('_id' in local && /^[0-9a-fA-F]{24}$/.test(local._id)) {
