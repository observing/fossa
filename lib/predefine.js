...
module.exports = function predefined(Base) {
  ...
  readable('client', function client(done) {
    this.fossa.connect(this._database, this.urlRoot || this.name, done);
  });
  ...
  readable('setup', function setup(hooks) {
    ...
    /**
     * Iterator to execute items on the stack synchronously.
     *
     * @param {Object} item
     * @returns {Mixed}
     * @api private
    ...
    function execute(item) {
      return model[item.fn].apply(model, item.value);
    }
    ...
    hooks.forEach(function each(hook) {
      ...
      for (var key in source) {
        ...
        //
        if (!~model._crud.indexOf(on)) continue;
        actions[on] = actions[on] || [];
        actions[on].push({
          fn: source[key],
          value: model.get(attrib),
          hook: hook
        });
      }
      ...
      Object.keys(actions).forEach(function register(action) {
        ...
        model.on(hook + ':' + action, function trigger(done) {
          async.each(actions[action], function execute(item, next) {
            model[item.fn].call(model, item.value, next);
          }, done);
        });
      });
    });
    ...
    model.validate = function validate(attributes, options) {
      if (stack.before) while (i = --stack.before.length) execute(stack.before[i]);
      validator.apply(model, arguments);
      if (stack.after) while (i = --stack.after.length) execute(stack.after[i]);
    };
  });
