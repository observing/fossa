...
backbone.sync = function sync(method, item, options) {
  ...
  function persist(client, next) {
    switch (method) {
      case 'create':
        var data = single ? item.attributes : _.pluck(item.models, 'attributes');
        client.insert(data, options, function inserted(error, results) {
      ...
      case 'read':
        ...
        //
        if (item.isNew()) return next(null, null);

        client.findOne({ _id: item.id }, options, function found(error, results) {
          if (error) return next(error);
      ...
      case 'update':
      case 'patch':
        var update = { $set: item.changed };
        if (options.upsert) update = item.attributes;

        client.update({ _id: item.id }, update, options, function updated(error, results) {
          if (error) return next(error);

          //
          // Update the _stored property for future reference.
          //
          if (results) item._stored = true;
          next(null, results);
        });
      break;

      case 'delete':
        client.remove({ _id: item.id }, options, function updated(error, results) {
          if (error) return next(error);
