...
module.exports = function predefined(Base) {
  ...
  readable('setup', function setup(hook) {
    ...
    //
    function initiate(stack) {
      return function trigger(done) {
        async.each(stack, function execute(item, next) {
...
backbone.sync = function sync(method, model, options) {
  ...
  //
  // Get the client and execute persistence logic.
  //
